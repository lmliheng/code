...
result_Group1 = zeros(sum_time, 7+4+7);
...
for calculate_time = 1:sum_time
    ...
    % 生成每次随机中的扰动
    for i = 1:7
        result_Group1(calculate_time, i) = rand(1) - 0.5;
        data_array(i) = data_matrix(i, result_group1(i)) + result_Group1(calculate_time, i) * 340 / 1000 ;
    end
    ...
    for opt_time = 1:20
        ...
        if exitflag > 0
            ...
            % if fitted_point(4) / 340 < opt_t
                ...
                temp_result_Group1 = zeros(1, 7+4+7);

                temp_result_Group1(1, 8) = fitted_point(1) / 97.304 + 110.241;
                temp_result_Group1(1, 9) = fitted_point(2) / 111.263 + 27.204;
                temp_result_Group1(1, 10) = fitted_point(3) * 1000;
                temp_result_Group1(1, 11) = fitted_point(4) / 340;
                ...
                for index = 1:size(data4, 1)
                    ...
                    temp_result_Group1(1, 11+index) = actual_distance - target_distance;
                end

                if sum(abs(temp_result_Group1(:, 12:18)), 2) ~= 0
                    if sum(abs(temp_result_Group1(:, 12:18)), 2) < opt_R_abs
                        result_Group1(calculate_time, 8:18) = temp_result_Group1(1, 8:18);
                        opt_R_abs = sum(abs(temp_result_Group1(:, 12:18)), 2);
                    end
                end
...
%% 保存结果
final_result_Group1 = result_Group1;
final_result_Group1(:, 19) = sum(abs(final_result_Group1(:, 12:18)), 2);

T = array2table(final_result_Group1);
...
filename = 'final_result_Group1.xlsx';

writetable(T, filename);

writetable(T, filename, 'Sheet', 'Sheet1');

disp('结果已保存到final_result_Group1.xlsx中')
%%
% 目标函数
